...
def validateUser(request, readerForm, userForm):
    ...
    return render(request, "librarian/createNewUser.html", context)
...
def createNewUser(request):
    readerForm = ReaderForm()
    userForm = UserForm()
    return render(request, "librarian/createNewUser.html", {"readerForm": readerForm, "userForm": userForm})
...
def librarian(request):
    reader = Reader.objects.get(user_id=request.user.id)
    libraryBooks = Book.getLibraryBooksJSON()
    context = {
        "reader": reader,
...
def profile(request):
    if request.method == "POST":
        ...
        if profileForm.errors:
            ...
            url = "librarian/createNewUser.html"
